...
class SongData(Dataset):
    ...
    def __init__(self, max_length=4, random_sample=True, item_size = 4):
...
class LyricWriter(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_size=225):
        super().__init__()
...
def train_nn(
    ...
    model = LyricWriter(vocab_size=len(dataset.token2idx)).to(device)
    ...
    for epoch in range(epochs):
        ...
        print(complete_string("I will nap under moonlight skies with you\n"))
    print("Training complete.")


def predict_next_string(model, prefix, token2idx, idx2token, max_len=30, temperature=0.5):
    """
...
def complete_string(prefix, model_path="model.pt", temperature=0.15):
    ...
    model = LyricWriter(vocab_size=len(token2idx))
